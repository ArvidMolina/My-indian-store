...
<form class="panel form-horizontal" action="{$currentIndex|escape:'htmlall':'UTF-8'}&token={$currentToken|escape:'htmlall':'UTF-8'}" name="fmm_registration_fields_form" id="fmm_registration_fields_form"   method="post" enctype="multipart/form-data">
	...
	{if $version >= 1.6}
		<div class="panel-footer">
			<a href="{$link->getAdminLink('AdminRegistrationFields')|escape:'html':'UTF-8'}" class="btn btn-default"><i class="process-icon-cancel"></i> {l s='Cancel' mod='registrationfields'}</a>
			<button type="submit" name="submitAddfmm_registration_fields" class="btn btn-default pull-right"><i class="process-icon-save"></i> {l s='Save' mod='registrationfields'}</button>
			<button type="submit" name="submitAddfmm_registration_fieldsAndStay" class="btn btn-default pull-right"><i class="process-icon-save"></i> {l s='Save and stay' mod='registrationfields'}</button>
		</div>
		{else}
		    <div style="text-align:center">
				<input type="submit" value="{l s='Save' mod='registrationfields'}" class="button" name="submitAddfmm_registration_fields" id="{$table|escape:'htmlall':'UTF-8'}_form_submit_btn" />
			</div>
		{/if}
</form>
