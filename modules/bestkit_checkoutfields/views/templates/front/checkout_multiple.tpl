...
{if count($checkout_field.options)}
<div class="checkout_field {if $checkout_field.required}required{/if} {$checkout_field.field_type|escape:'htmlall':'UTF-8'} form-group">
    ...
    <select multiple="multiple" name="checkout_field[{$checkout_field.id_bestkit_checkout_field|escape:'htmlall':'UTF-8'}]" id="checkout_field_{$checkout_field.id_bestkit_checkout_field|intval}" attr-id="{$checkout_field.id_bestkit_checkout_field|intval}" class="form-control checkout_field_value checkout_field_step_{$checkout_field_step|escape:'htmlall':'UTF-8'}">
		<option value="">{l s='--' mod='bestkit_checkoutfields'}</option>
        {foreach $checkout_field.options as $option}
        <option value="{$option.value|escape:'htmlall':'UTF-8'}" {if is_array($checkout_field.selected_value) && in_array($option.value, $checkout_field.selected_value)}selected="selected"{/if}>{$option.value|escape:'htmlall':'UTF-8'}</option>
        {/foreach}
    </select>
</div>
{/if}