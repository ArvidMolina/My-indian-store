...
class VenipakCarrier extends CarrierModule {
    ...
    public function sendersList()
    {
        ...
        //collect all current warehouses
        self::checkForClass('VenipakDatabaseHelper');
        $warehouses = VenipakDatabaseHelper::getSenders();
        if(!empty($warehouses)){
            foreach($warehouses as $wh){
                $content[] = array(
                    'VENIPAK_ADDRESS_ID'=>$wh['id'],
    ...
    public function saveAddress(){
        ...
        if(Tools::getValue('VENIPAK_ADDRESS_ID')==false){
            ...
            if(Tools::getValue('VENIPAK_ADDRESS_IS_DEFAULT')=='1'){
                Db::getInstance()->update('venipak_carrier_addresses', array('is_default'=>'0'), 'is_default=1 AND type="'.$address_type.'"');
        ...
        if (!VenipakDatabaseHelper::hasDefaultAddress($address_type))
            Db::getInstance()->update('venipak_carrier_addresses', array('is_default'=>'1'), 'is_default=0 AND type="'.$address_type.'" LIMIT 1');
    ...
    public function hookDisplayAdminOrder($params)
    {
        ...
        // checking addresses
        if (!VenipakDatabaseHelper::hasDefaultAddress('warehouse') || !VenipakDatabaseHelper::hasDefaultAddress('sender'))
        {
        ...
        $this->smarty->assign(array(
            ...
            'venipak_warehouses'=>VenipakDatabaseHelper::getWarehouses(),
            'venipak_senders'=>VenipakDatabaseHelper::getSenders(),
    ...
    public function saveCarrierCall($warehouseId, $callDate, $callData){
