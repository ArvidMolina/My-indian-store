...
class ElprestaUpdater
{
    ...
    private static function getNewestVersion($module)
    {
                        ...
                        if (!empty($response_json))
                        {
                            ...
                            if (isset($response['new_version']) && !empty($response['new_version']))
                            {
                                ...
                                if (version_compare($module->version, $response['new_version'], '='))
                                    return $module->displayConfirmation($module->l('Successfully checked for updates. No new updates were found. You are using latest module version.'));
                            }
                        }

                    }
                    else
                        return $module->displayConfirmation($module->l('Unable to check for updates. Funcion `file_get_contents` is disabled.'));
                }
                else
                    return $module->displayConfirmation($module->l('Unable to check for updates. Setting `allow_url_fopen` is disabled.'));
            }
        ...
        {
            Configuration::updateValue(strtoupper($module->name . '_last_update_check_date'), date("U"));
            return $module->displayConfirmation($module->l('Failed to check for updates. Error: '.$ex->getMessage()));
        }
    }
