...
class VenipakAPI {
    ...
    public function sendShipmentXML($orderIds){
        $db = Db::getInstance();
        ...
        $warehousesOriginal = VenipakDatabaseHelper::getWarehouses();
        $warehouses = array();
        //reformat
        ...
        $consignorsOriginal = VenipakDatabaseHelper::getSenders();
        $consignors = array();
        //reformat
        ...
        foreach($allOrders as $manifestNo => $orders){
            ...
            foreach($orders as $order_id => $order){
                ...
                if(!empty($order['consignor_id']) && $order['consignor_id']!='0' && $order['consignor_id']!=''){
                    ...
                    $consignor->addChild('post_code', $consignors[$order['consignor_id']]['postcode']);
                ...
                $sender->addChild('post_code', $warehouses[$order['sender_id']]['postcode']);
        ...
        $data = array('user'=>Configuration::get('VENIPAK_API_LOGIN'), 'pass' => Configuration::get('VENIPAK_API_PASSWORD'), 'xml_text' => $xmlText);
    ...
    public function getLabelsForOrders($orderIds){
        ...
        if(!empty($packNos)){
            ...
            foreach($packsSorted as $manifest => $packs){
                $data = array(
    ...
    public function callCourier($warehouseId, $params){
        ...
        $sender->addChild('post_code', $warehouse['postcode']);
