...
class VenipakDatabaseHelper
{
    ...
    public static function getWarehouses(){
        $db = Db::getInstance();
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'venipak_carrier_addresses WHERE `type`=\'warehouse\' ORDER BY is_default DESC, id ASC';
    ...
    public static function getDefaultWarehouse(){
        $db = Db::getInstance();
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'venipak_carrier_addresses WHERE `type`=\'warehouse\' AND is_default = 1';
        return $db->getRow($sql);
    }

    public static function getSenders(){
        $db = Db::getInstance();
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'venipak_carrier_addresses WHERE `type`=\'sender\' ORDER BY is_default DESC, id ASC';
        return $db->executeS($sql);
    }
    ...
    public static function hasDefaultAddress($type){
        $db = Db::getInstance();
        $sql = 'SELECT 1 FROM ' . _DB_PREFIX_ . 'venipak_carrier_addresses WHERE `type`=\''.$type.'\' and is_default = 1';
    ...
    public static function getCarrierCallByDate($warehouseId, $date){
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'venipak_carrier_call WHERE date(call_date)="'.pSQL(strval($date)).'" AND warehouse_id='.(int)$warehouseId;
        return Db::getInstance()->getRow($sql);
    }

    public static function getCarrierCallFromDate($warehouseId, $date){
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'venipak_carrier_call WHERE date(call_date)>="'.pSQL(strval($date)).'" AND warehouse_id='.(int)$warehouseId;
