...
class VenipakcarrierajaxModuleFrontController extends ModuleFrontController {
    ...
    protected function renderPopup()
    {
        ...
        $warehouses = VenipakDatabaseHelper::getWarehouses();
        $manifests = array();
        ...
        foreach($warehouses as $wh){
            ...
            $courrierCalls[$wh['id']] = VenipakDatabaseHelper::getCarrierCallFromDate($wh['id'], date('Y-m-d'));
    ...
    protected function saveVenipakOrder(){
        ...
        $saveResult = $venipakOrderObj->saveVenipakOrder();
        ...
        if(isset($saveResult['success'])){
            ...
            $venipakResult = $venipakAPI->sendShipmentXML(array($saveResult['success']));
            if(is_array($venipakResult) && isset($venipakResult['success'])){
                ...
                if (!empty($trackingNo)) {
                    ...
                    if (Configuration::get('VENIPAK_TRACKING_ACTION')) {
                        ...
                        $order_carrier = new OrderCarrier((int)$orderObj->id_carrier);

                        $order_carrier->tracking_number = pSQL($trackingNo);
                        $orderObj->shipping_number = $trackingNo;
                        $orderObj->save();
                        $order_carrier->save();

                        if (version_compare(_PS_VERSION_, '1.7', '>='))
                        {
                            $order_carrier->sendInTransitEmail($orderObj);
                        }
    ...
    protected function closeManifest()
    {
        ...
        header('Location: '.__PS_BASE_URI__.'office/'.$context->link->getAdminLink('AdminVenipak', true).'&conf=5');
    }
    ...
    protected function callCarrier()
    {
        ...
        if($testDate->format($format)!=date($format)){
            ...
            $checkFutureCall = VenipakDatabaseHelper::getCarrierCallByDate($warehouse_id, $testDate->format($format));
        ...
        if(!empty($result) && isset($result['success'])){
            //insert in database
            $insertResult = $this->_module->saveCarrierCall($warehouse_id, $testDate->format('Y-m-d'), $dataArrayEncoded);
        }
