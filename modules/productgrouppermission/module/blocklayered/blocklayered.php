...
class BlockLayered extends Module
{
	...
	public function getProductByFilters($selected_filters = array())
	{
		...
		$query_filters_where = ' AND '.$alias_where.'.`active` = 1 AND '.$alias_where.'.`visibility` IN ("both", "catalog") AND pp.permission_customer_group_id = '.$context->customer->id_default_group.'';
		$query_filters_from = '';
		...
		if (empty($selected_filters['category']))
		{
			...
			Db::getInstance()->execute('CREATE TEMPORARY TABLE '._DB_PREFIX_.'cat_filter_restriction ENGINE=MEMORY
														...
														LEFT JOIN '._DB_PREFIX_.'product_permission pp ON (pp.permission_product_id = cp.id_product)
		...
		if (isset($price_filter) && $price_filter) {
			...
			if (empty($selected_filters['category'])) {
				...
				FROM `'._DB_PREFIX_.'product` p
				LEFT JOIN '._DB_PREFIX_.'product_permission pp ON (pp.permission_product_id = p.id_product)
				 JOIN '._DB_PREFIX_.'category_product cp USING (id_product)
				'.$price_filter_query_out.'
				'.$query_filters_from.'
		...
		else
		{
			...
			if (version_compare(_PS_VERSION_, '1.6.1', '>=') === true)
			{
				...
				LEFT JOIN '._DB_PREFIX_.'product_permission pp ON (pp.permission_product_id = cp.id_product)
				WHERE '.$alias_where.'.`active` = 1 AND '.$alias_where.'.`visibility` IN ("both", "catalog") AND pp.permission_customer_group_id = '.$context->customer->id_default_group.'
	...
	public function getFilterBlock($selected_filters = array())
	{
		...
		Db::getInstance()->execute('CREATE TEMPORARY TABLE '._DB_PREFIX_.'cat_restriction ENGINE=MEMORY
													...
													LEFT JOIN '._DB_PREFIX_.'product_permission pp ON (pp.permission_product_id = cp.id_product)
													WHERE product_shop.`active` = 1 AND product_shop.`visibility` IN ("both", "catalog") AND pp.permission_customer_group_id = '.$context->customer->id_default_group, false);
		...
		foreach ($filters as $filter)
		{
			...
			switch ($filter['type'])
			{
				...
				case 'category':
					...
					AND '.$alias.'.active = 1 AND '.$alias.'.`visibility` IN ("both", "catalog") AND pp.permission_customer_group_id = '.$context->customer->id_default_group;
