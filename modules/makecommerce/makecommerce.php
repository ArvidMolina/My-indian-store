...
class MakeCommerce extends PaymentModule
{
    ...
    public function confirmOrder($payment_methods = null, $cart_info = null)
    {
		...
		if ($cart_info->shipmentMethod->carrier == 'OMNIVA')
		    ...
		    $sco_address->postcode = $terminal['zip'];
    ...
    private function _doSCO()
    {
        ...
        $mk_cart = array(
            ...
            'amount' => number_format(round($cart->getOrderTotal(true, 1), 2), 2), // number_format(round(Tools::ps_round($cart->getOrderTotal(true, 1), 2),2),2),
            'currency' => 'EUR',
        ...
        Db::getInstance()->Execute(
            'INSERT INTO `'._DB_PREFIX_.'makecommerce_sco` (`id_cart`, `sco_id`)
                    VALUES('.(int)$cart->id.', \''.$sco->id.'\')'
        );
