                    ...
                    {foreach $kb_available_field as $kbfield}
                        ...
                        {if ($kbfield['type'] == 'date') && ($section['id_section'] == $kbfield['id_section'])}
                            <script>
                                ...
                                $('input[name="{$kbfield['field_name']}"]').datepicker();
                                $('input[name="{$kbfield['field_name']}"]').on('keypress keydown keyup', function (e) {
                                    e.preventDefault();
                                    return false;
                                });
                            </script>
                        ...
                        {if ($kbfield['type'] == 'file') && ($section['id_section'] == $kbfield['id_section'])}
                            ...
                            <div class="form-group">
                                <label class="control-label col-lg-3" for="{$kbfield['html_id']}">
                                    <span  {if $kbfield['description'] != ''}class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="{$kbfield['description']|escape:'htmlall':'UTF-8'}"{else}class="control-label"{/if}>{$kbfield['label']|escape:'htmlall':'UTF-8'}</span>
                                </label>
                                <div class="col-lg-4">
                                    {if isset($kbfield['customer_value'])}
                                        <div>
                                            {if is_array($kbfield['customer_value']|json_decode:1) && $kbfield['customer_value'] != ''} 
                                                <a style="padding: 4px;margin-bottom: 6px" class="btn btn-warning" href="{$kb_front_controller}?downloadFile=true&id_field={$kbfield['id_field']}&id_customer={$id_customer}" ><i class="icon-download"></i> {l s='Download File' mod='kbcustomfield'}</a>
                                            {/if}
                                        </div>
                                    {/if}
                                    {if $kbfield['editable']}
                                        ...
                                        {if $kbfield['file_extension'] != ''} <span class="form-info ">{l s='File must be ' mod='kbcustomfield'}<span class="file_extension">{$kbfield['file_extension']}</span></span></br>{/if}
                                        {/if}
                                </div>
                            </div>
