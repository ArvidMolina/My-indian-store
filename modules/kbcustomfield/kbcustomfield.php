...
class Kbcustomfield extends Module
{
    ...
    */
    ...
    {
        if (($this->context->controller->php_self == 'authentication') || (($this->context->controller->php_self == 'identity'))) {
            $this->context->controller->addJquery();
            $this->context->controller->addJqueryUI('ui.slider');
             $this->context->controller->addCSS($this->_path . 'views/css/front/kb_front.css');
             $this->context->controller->addJQueryUi('ui.datepicker');
//            $this->context->controller->addJS($this->_path . 'views/js/jquery.ui.datepicker.min.js');
...
//                                    'kb_redirect_error', $this->l('Something went wrong while updating the information. Please try again.')
//                            );
                        }
                    }
                }
    ...
    {
        if (version_compare(_PS_VERSION_, '1.7.0.9', '<=')) {
            $kbconfig = Tools::jsonDecode(Configuration::get('KB_CUSTOM_FIELDS'), true);
            if ($kbconfig['enable']) {
