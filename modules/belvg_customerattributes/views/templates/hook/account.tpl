				...
				{foreach from=$belvg_attr_section->attributes item=attribute}
					...
					<div class="form-group {if $attribute->required eq 1}required {/if}{if $attribute->type eq 'radio'} {elseif $attribute->type eq 'textarea'} textarea{elseif ($attribute->type eq 'multiselect' || $attribute->type eq 'dropdown')}select{else}text{/if}">
						...
						{/if}
							...
							<input type="text" class="text form-control" id="{$input_name|escape:'htmlall':'UTF-8'}" name="{$input_name|escape:'htmlall':'UTF-8'}" value="{$customer_value|escape:'htmlall':'UTF-8'}">
							{elseif $attribute->type eq 'textarea'}
							<textarea rows="6" name="{$input_name|escape:'htmlall':'UTF-8'}" id="{$input_name|escape:'htmlall':'UTF-8'}" class="form-control">{$customer_value|escape:'htmlall':'UTF-8'}</textarea>
							{elseif $attribute->type eq 'date'}
							<input type="text" class="text belvg_date form-control" id="{$input_name|escape:'htmlall':'UTF-8'}" name="{$input_name|escape:'htmlall':'UTF-8'}" value="{$customer_value|escape:'htmlall':'UTF-8'}">
