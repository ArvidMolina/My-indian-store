...
<div class="pos-menu-vertical" data-more-less="{$menu_config.posvegamenu_more_less}">
	...
	<ul class="menu-content"> 
		{foreach from=$menus item=menu name=menus}	 			
				<li class="menu-item menu-item{$menu.id_posvegamenu_item} {$menu.class|escape:'html':'UTF-8'}{if count($menu.sub_menu) > 0}hasChild{/if} {if isset($menu.selected_item) && $menu.selected_item == 1}active{/if}">
					...
					<a href="{if $menu.link}{$menu.link|escape:'html':'UTF-8'}{elseif $menu.custom_link}{$menu.custom_link|escape:'html':'UTF-8'}{else}javascript:void(0){/if}" {if $menu.new_window == 1} target="_blank" {/if}>
						...
						{if count($menu.sub_menu) > 0} <i class="hidden-md-down fa-angle-right"></i>{/if}
					</a>
					...
					{if $menu.submenu_type == 0}
						...
						<div class="pos-sub-menu menu-dropdown col-xs-12 col-lg-{$menu.sub_menu.submenu_config.submenu_width|escape:'html':'UTF-8'} {$animation_class}">
							<div class="popup_vertical">
							{foreach from=$menu.sub_menu.info_rows item= menu_row name=menu_row}
								<div class="pos-menu-row row {$menu.class|escape:'html':'UTF-8'}">
															...
															{else if $sub_menu_item.type_link == 5}
																{if $sub_menu_item.image}
																	{if $sub_menu_item.image_link}
																		<a href="{$sub_menu_item.image_link}"><img src="{$sub_menu_item.image}" /></a>
																	{else}
